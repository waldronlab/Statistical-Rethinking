---
title: "An Introduction to Markov Monte Carlo Estimation"
...
```{r fig.height=6,echo=FALSE, fig.align='center'}
img <- readPNG("img/king_markov_plot.png")
 grid.raster(img)
```
...
- The “population sizes” in our objective are the posterior probabilities at each parameter
value.
- The “weeks” in our objective are samples taken from the joint posterior of the parameters
in the model.
...
> - sampling takes longer than other techniques
> - escaping the akwardness of assuming multivariate normality
...
pairs.panels(as.data.frame(post.unif), pch=21)
```
...
pairs.panels(as.data.frame(post.exp), pch=21)
```
...
- The intervall is a highest posterior density intervall (HPDI)
...
```{r fig.align='center', fig.width=8, fig.cap='Decreasing Scaling Parameter for Cauchy Distribution'}
...
legend(2, 0.8, legend=c("10", "1", "0.1"), col=c("black", "blue", "red"), lty=1:1, cex=0.8, title = "Prior Cauchy Scale")
```
...
legend(6, 0.8, legend=c("10", "1", "0.1"), col=c("black", "blue", "red"), lty=1:1, cex=0.8, title = "Prior Exponential Scale")
```
...
img <- readPNG("img/non_identifiable_parameters.png")
 grid.raster(img)
```

We can not simultaneously estimate a1 and a2 but only the sum.
Weak priors will help here as well.

## Exercise 8H2 Compare different models

*Recall the divorce rate example from Chapter 5. Repeat that analysis, using map2stan this*
*time, fitting models m5.1, m5.2, and m5.3. Use compare to compare the models on the basis of*
